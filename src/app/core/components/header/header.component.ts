...
export class HeaderComponent implements OnInit {
  ...
  logout() {
    concat(this.session.logout(), this.auth.getClientSession()).subscribe(() => {
      this.snackBar.open('Você foi desconectado.', 'Close', { duration: 4000});
      this.session.setLoggedInStatus(false);
    },
      err => this.snackBar.open('Ocorreu um problema ao fazer o seu logout.', 'Close', { duration: 4000}));
  }
