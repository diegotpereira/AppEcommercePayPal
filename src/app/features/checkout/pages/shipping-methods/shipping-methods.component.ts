...
export class ShippingMethodsComponent implements OnInit {
    ...
    combineLatest([
      ...
      this.shipments = values[0].shipments?.map((shipment) => {
        ...
        if (shipment.availableShippingMethods) {
          ...
          shipment.availableShippingMethods = shipment.availableShippingMethods?.map(method => {
  ...
  setShipmentMethods() {
    ...
    err => this.snackBar.open('Ocorreu um problema ao adicionar métodos de envio às suas remessas.', 'Close', { duration: 5000}));
  }
